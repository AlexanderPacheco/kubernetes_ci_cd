...
const BASE_URL = process.env.API_MICROSERVICIOLISTARCARPETAS //'http://localhost:3400'
...
router.get('/func/g2/listarcarpetas/:nickname', (req, res) => {
    let pack = log;
    api.get(req.path).then(resp => {
        ...
        try {
            ...
            pack.entrada =  req.params.nickname;
            pack.fechaHora = fecha();
            pack.salida = "Carpeta: "+resp.data;
            pack.esError = "no";
            res.send(resp.data);
        } catch (err) {
            console.error(err);
